...
from app.models.storage import Product, ProductCreate, ProductPublic, ProductUpdate
...
async def create_product(
    product_in: ProductCreate, session: AsyncSession = Depends(get_session)
):
    product = await crud_product.get(session, title=product_in.title)
    if product:
        raise HTTPException(status_code=409, detail="this product already exist")
    obj_in = ProductCreate(**product_in.model_dump())
...
async def update_product(
    storage_id: int,
    storage_in: ProductUpdate,
    session: AsyncSession = Depends(get_session),
):
    storage = await crud_product.get(session, id=storage_id)
    if not storage:
        raise HTTPException(status_code=404, detail="this storage doesnt exist")
    try:
        storage = await crud_product.update(
            session,
            db_obj=storage,
            obj_in={
        ...
        raise HTTPException(status_code=409, detail="this storage already exist")
    return storage
