...
from app.models.storage import Storage, StorageCreate, StoragePublic, StorageUpdate
...
async def create_storage(
    storage_in: StorageCreate, session: AsyncSession = Depends(get_session)
) -> Storage:
    storage = await crud_storage.get(session, title=storage_in.title)
    if storage:
        raise HTTPException(status_code=409, detail="storage already exist")
    obj_in = StorageCreate(**storage_in.model_dump())
...
async def update_storage(
    ...
    if not storage:
        raise HTTPException(status_code=404, detail="storage not found")
    try:
        storage = await crud_storage.update(
            session,
            db_obj=storage,
            obj_in={
